...
// figures out who they most likely are so we can attach their canonical
// name, email, and roles to the submission — without ever showing the
...
/**
 * Matches free-text input against the roster.
 * Returns { matchedPerson, confidence, candidates } where:
 *   confidence: "exact" | "fuzzy" | "partial" | "ambiguous" | "none"
 *   candidates: up to 3 best guesses (for admin review in Firestore),
 *               each { name, score }
 */
export function matchName(rawInput, roster) {
  const input = normalize(rawInput);
  if (!input) return { matchedPerson: null, confidence: "none", candidates: [] };

  const inputTokens = input.split(" ");
  const scored = roster.map((person) => {
  ...
  const candidates = scored.slice(0, 3).map((s) => ({ name: s.person.name, score: Number(s.score.toFixed(3)) }));

  if (!best || best.score < 0.5) {
    return { matchedPerson: null, confidence: "none", candidates };
  }

  if (best.score >= 0.97) {
    return { matchedPerson: best.person, confidence: "exact", candidates };
  }

  // Ambiguous if the top two candidates are close to each other.
  if (second && best.score - second.score < 0.08 && best.score < 0.85) {
    return { matchedPerson: best.person, confidence: "ambiguous", candidates };
  }

  if (best.score >= 0.75) {
    return { matchedPerson: best.person, confidence: "fuzzy", candidates };
  }

  return { matchedPerson: best.person, confidence: "partial", candidates };
}
